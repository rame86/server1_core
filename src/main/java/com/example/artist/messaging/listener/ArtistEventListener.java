...
public class ArtistEventListener {
	...
	public void handlePaymentResult(PaymentResponseDTO response) {
		log.info("=====> [Artist 서버 수신] 주문번호: {}, 상태: {}, 메시지: {}", 
                response.orderId() , response.status(), response.message());
		
		// 주문번호로 DB에서 주문 정보 찾아오기
		Donation donation = donationRepository.findByOrderId(response.orderId())
				.orElseThrow(() -> new RuntimeException("해당 주문을 찾을 수 없습니다 : " + response.orderId()));
		
		// 응답 상타에 따라 DB의 status를 업데이트
		if("COMPLETE".equals(response.status())) {
			donation.complete();
			log.info("----> 주문번호 {} 후원 처리 완료(DB업데이트 완료)", response.orderId());
		} else if("FAIL".equals(response.status())){
			donation.fail();
			log.info("----> 주문번호 {} 후원 처리 실패(DB업데이트 완료){}", response.orderId(), response.message());
		}
	}
